...
[_LOWER] = LAYOUT_planck_grid(
    ...
    RGB_MODE_BREATHE, _______, _______, _______, _______, _______, _______, _______,    KC_HOME,    KC_PGDN, KC_PGUP, KC_END
),
...
bool led_update_user(led_t led_state) {
    //uprintf("here in led update with the state of caps: %s and also the USB thing is: %s and the function is: %s\n", led_state.caps_lock, USB_LED_CAPS_LOCK, host_keyboard_leds());
    uprintf("here in led update with the state=%s\n", led_state);
 /*   if (host_keyboard_leds() & (1<<USB_LED_CAPS_LOCK)) {
       printf("ini it");
         rgblight_set_layer_state(0, true);// led_state.caps_lock);
    }*/
    return true;
}
